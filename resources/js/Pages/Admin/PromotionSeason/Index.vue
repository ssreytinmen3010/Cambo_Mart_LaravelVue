...
async function handleImageUpload(event) {
    ...
    try {
        ...
        if (error.response?.data?.message) {
            imageUploadError.value = error.response.data.message;
        } else {
            imageUploadError.value = "The image failed to upload.";
        }
    }
}
...
<template>
    <AdminLayout
        ...
        subtitle="Seasonal coupons and campaigns"
    >
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm">
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="divide-y divide-slate-50">
                    <tr
                        v-for="item in filteredSeasons"
                        :key="item.id"
                        class="hover:bg-slate-50/50 transition-colors"
                    >
                        <td class="px-6 py-4">
                            <div
                                class="text-sm font-mono font-bold text-slate-700 bg-slate-100 px-2.5 py-1.5 rounded inline-block"
                            >
                                {{ item.code }}
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="w-16 h-16">
                                ...
                                <div
                                    v-else
                                    class="w-16 h-16 rounded-lg border border-dashed border-slate-200 bg-slate-50 flex items-center justify-center text-slate-300"
                                >
                                    <v-icon size="22"
                        ...
                        <td class="px-6 py-4 text-sm text-slate-600">
                            <div class="font-bold text-slate-700">
                                {{ promotionTypeLabel(item.promotion_type) }}
                            </div>
                            <div class="text-xs text-slate-400">
                                ${{
                                    Number(item.promotion_value || 0).toFixed(2)
                                }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            ...
                            >
                                ...
                                <span
                                    ...
                                    class="px-3 py-1.5 rounded-full text-xs font-black uppercase tracking-tighter"
                                >
                                    {{ item.status }}
                                </span>
                            </div>
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            class="w-8 h-8 rounded-lg bg-rose-50 flex items-center justify-center"
                                        >
                                            <v-icon size="16"
        ...
        <Transition name="modal-fade">
            ...
            >
                ...
                >
                    <div
                        class="px-8 py-6 shrink-0 flex items-center justify-between border-b border-slate-100"
                    >
                        <h2
                        ...
                        >
                            {{
                                isEdit
                                    ? "Edit Promotion Season"
                                    : "New Promotion Season"
                            }}
                        </h2>
                        <button
                            type="button"
                            ...
                            @click="showModal = false"
                        >
                            <v-icon size="20" class="text-slate-400"
                                >mdi-close</v-icon
                            >
                        </button>
                    </div>
                    <div class="p-8 space-y-4 overflow-y-auto custom-scrollbar">
                        <div class="space-y-1">
                            ...
                            <div
                                ...
                                class="text-rose-500 text-[10px] ml-2 font-bold"
                            >
                                {{ form.errors.code }}
                            </div>
                        </div>
                        <div class="space-y-1">
                            ...
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-20 h-20 rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center"
                                >
                                    <img
                                        v-if="previewImage"
                                        :src="previewImage"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button
                                        type="button"
                                        class="px-4 py-2.5 bg-white border border-slate-200 rounded-2xl text-xs font-black text-slate-600 hover:bg-slate-50"
                                    ...
                                    <button
                                        v-if="previewImage"
                                        type="button"
                                        class="px-4 py-2.5 bg-rose-50 border border-rose-100 rounded-2xl text-xs font-black text-rose-600"
                                        @click="clearImage"
                                    >
                                        Remove
                                    </button>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="space-y-1">
                                ...
                                <div
                                    v-if="form.errors.promotion_type"
                                    class="text-rose-500 text-[10px] ml-2 font-bold"
                                >
                                    {{ form.errors.promotion_type }}
                                </div>
                            </div>
                            <div class="space-y-1">
                                <label
                                ...
                                />
                                <div
                                    v-if="form.errors.promotion_value"
                                    class="text-rose-500 text-[10px] ml-2 font-bold"
                                >
                                    {{ form.errors.promotion_value }}
                                </div>
                            </div>
                        </div>
                        <div class="space-y-1">
                    ...
                    <div
                        class="px-8 py-6 bg-slate-50/50 border-t border-slate-100 flex items-center justify-end gap-3 shrink-0"
                    >
                        <button
                            type="button"
                            class="px-6 py-2.5 text-xs font-bold text-slate-400 hover:text-slate-600 uppercase tracking-widest"
                            @click="showModal = false"
                        >
                            Cancel
                        </button>
        ...
        <Transition name="modal-fade">
            ...
            >
                ...
                ></div>
                <div
                    class="relative bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden animate-pop flex flex-col"
                >
                    <div
                        class="px-8 py-6 flex items-center justify-between border-b border-slate-100"
                    >
                        <h2 class="text-xl font-black text-slate-800">
                            Promotion Season Details
                        </h2>
                        <button
                            type="button"
                            class="w-10 h-10 flex items-center justify-center rounded-xl bg-emerald-50 hover:bg-emerald-100"
                            @click="showViewModal = false"
                        >
                            <v-icon size="20" class="text-emerald-600"
                                >mdi-close</v-icon
                            >
                        </button>
                    </div>
                    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-4">
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-20 h-20 rounded-2xl border overflow-hidden bg-slate-50 flex items-center justify-center"
                                >
                                    <img
                                        v-if="viewItem?.image"
                                        :src="viewItem.image"
                                ...
                                <div>
                                    <div
                                        class="text-xs font-mono font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded inline-block mb-1"
                                    >
                                        {{ viewItem?.code }}
                                    </div>
                                    <p class="text-sm text-slate-500">
                                        Created {{ viewItem?.created_at }}
                                    </p>
                                </div>
                            </div>
                            <div
                            ...
                            >
                                ...
                                >
                                <span
                                    :class="statusBadgeClass(viewItem?.status)"
                        ...
                        <div class="space-y-4">
                            ...
                            >
                                ...
                                <span
                                    class="text-sm font-bold text-slate-700"
                                    >{{ viewItem?.end_date || "—" }}</span
                                >
                            </div>
                        </div>
                    </div>
                    <div
                        class="px-8 py-6 bg-slate-50 border-t flex justify-end"
                    >
                        <button
                            type="button"
                            class="px-8 py-3 bg-emerald-600 text-white text-xs font-black rounded-2xl uppercase tracking-widest"
                            @click="showViewModal = false"
                        >
                            Close
                        </button>

...
function openEditModal(cat) {
    ...
    form.id = cat.id;
    form.name = cat.name;
    form.brand_ids = Array.isArray(cat.brand_ids)
        ? cat.brand_ids.map((id) => Number(id))
        : cat.brand_id
          ? [Number(cat.brand_id)]
          : [];
...
<template>
    <AdminLayout title="Categories" subtitle="Organize your items">
        ...
        <!-- Filters -->
        <div
            class="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-4 bg-white p-2 rounded-3xl border-slate-100 shadow-sm w-full md:w-1/2"
        >
            <div class="relative">
                <input
                    v-model="searchName"
                    type="text"
                    placeholder="Search..."
                    class="w-full pl-4 pr-4 py-1.5 bg-slate-50 border border-slate-100 rounded-full text-xs focus:ring-4 focus:ring-green-500/10 focus:border-green-400 outline-none transition-all"
                />
            </div>
            <div class="relative">
                <!-- This is just visual unless you implement server filtering or have all data -->
                <select
                    class="w-full pl-4 pr-8 py-1.5 bg-slate-50 border border-slate-100 rounded-full text-xs font-medium text-slate-600 outline-none appearance-none"
                >
                    ...
                    >
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-9 h-9 rounded-lg bg-blue-50 text-blue-500 flex items-center justify-center overflow-hidden"
                                >
                                    <img
                                        v-if="cat.image"
                                        :src="cat.image"
                ...
                >
                    ...
                    <div class="p-8 space-y-4">
                        ...
                        <div class="flex justify-center mb-2">
                            <div
                                class="relative w-16 h-16 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center overflow-hidden cursor-pointer"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="space-y-2 md:col-span-2">
                                ...
                                >
                                <div
                                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3"
                                >
                                    <label
                                        v-for="b in brands"
                                        :key="b.id"
                                        class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 shadow-sm transition hover:border-emerald-300"
                                    >
                                        <input
                        ...
                        <div class="space-y-1">
                            ...
                            <select
                                v-model="form.status"
                                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none appearance-none cursor-pointer text-sm"
                            >

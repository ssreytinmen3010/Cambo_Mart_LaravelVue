...
const form = useForm({
  ...
  image: "",
  status: true,
  status_stock: props.statusStocks && props.statusStocks.length > 0 ? props.statusStocks[0].id : "Sale"
});
...
watch(() => form.brand_id, (newBrandId) => {
  if (!isEdit.value) {
    ...
    const belongs = props.categories.find(c => c.id == form.category_id && c.brand_id == newBrandId);
...
watch(() => form.stock, (newStock) => {
  if (newStock !== "" && newStock !== null && typeof newStock !== 'undefined') {
...
function openAddModal() {
  ...
  form.reset();
  form.status = true;
  form.status_stock = props.statusStocks && props.statusStocks.length > 0 ? props.statusStocks[0].id : "Sale";
      ...
      <table class="min-w-full divide-y divide-slate-200">
        ...
        <tbody class="divide-y divide-slate-50">
          <tr v-for="p in filteredProducts" :key="p.id" class="hover:bg-slate-50/50 transition-colors">
            ...
            <td class="px-6 py-4">
               <span :class="p.status_stock === 'Sale' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'" class="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide">
                 {{ p.status_stock }}
               </span>
            </td>
            ...
            <td class="px-6 py-4 text-right">
              <div class="flex justify-end relative dropdown-container">
                <button @click.stop="toggleDropdown(p.id)" class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-all">
                  <v-icon size="20">mdi-dots-vertical</v-icon>
                </button>
                
                <div v-if="activeDropdown === p.id" class="absolute right-0 mt-10 w-40 bg-white rounded-xl shadow-xl border border-slate-100 z-50 py-1 overflow-hidden animate-pop">
                  <button @click="openViewModal(p)" class="w-full px-4 py-2.5 text-left text-xs font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-2">
                    <v-icon size="16" class="text-blue-500">mdi-eye-outline</v-icon> View Detail
                  </button>
                  <button @click="openEditModal(p)" class="w-full px-4 py-2.5 text-left text-xs font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-2">
                    <v-icon size="16" class="text-emerald-500">mdi-pencil-outline</v-icon> Edit Product
                  </button>
                  <div class="h-[1px] bg-slate-100 my-1"></div>
                  <button @click="deleteItem(p.id)" class="w-full px-4 py-2.5 text-left text-xs font-bold text-rose-500 hover:bg-rose-50 flex items-center gap-2">
    ...
    <Transition name="modal-fade">
      <div v-if="showViewModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-pop flex flex-col max-h-[90vh]">
          <div class="bg-gradient-to-tr from-blue-600 to-indigo-500 px-8 py-6 text-white shrink-0 flex justify-between items-center">
            <h2 class="text-xl font-black tracking-tight">Product Details</h2>
            <button @click="showViewModal = false" class="hover:rotate-90 transition-all">
          ...
          <div class="p-8 overflow-y-auto custom-scrollbar">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div class="space-y-6">
                <div class="aspect-square bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-center overflow-hidden shadow-inner">
                  <img v-if="viewProduct.image" :src="viewProduct.image" class="w-full h-full object-contain p-4">
                  <v-icon v-else size="100" class="text-slate-200">mdi-package-variant</v-icon>
                </div>
                
                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div class="text-[10px] font-bold text-slate-400 uppercase">Current Status</div>
                  <span :class="viewProduct.is_active ? 'bg-green-100 text-green-700' : 'bg-red-50 text-red-500'" class="px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-tighter shadow-sm">
                    {{ viewProduct.status }}
                  </span>
                </div>
              </div>
              ...
              <div class="space-y-5">
                <div>
                  <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Product Name</label>
                  <p class="text-lg font-black text-slate-800 leading-tight">{{ viewProduct.name }}</p>
                </div>

                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Code / SKU</label>
                    <p class="text-sm font-bold text-slate-600 font-mono">{{ viewProduct.code }}</p>
                  </div>
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Price</label>
                    <p class="text-lg font-black text-green-600">${{ viewProduct.price }}</p>
                  </div>
                </div>

                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Brand</label>
                    <span class="px-3 py-1 bg-slate-100 text-slate-700 rounded-lg text-xs font-bold">{{ viewProduct.brand }}</span>
                  </div>
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Availability</label>
                    <span :class="viewProduct.status_stock === 'Sale' ? 'bg-blue-100 text-blue-700' : 'bg-amber-100 text-amber-700'" class="px-3 py-1 rounded-lg text-xs font-bold">
                ...
                <div>
                  <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Stock Availability</label>
                  <div class="flex items-center gap-3">
                    <div class="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                      <div class="h-full bg-blue-500 rounded-full" :style="{ width: Math.min(viewProduct.stock, 100) + '%' }"></div>
                    </div>
                    <span class="text-sm font-black text-slate-700">{{ viewProduct.stock }} units</span>
                  </div>
                </div>

                <div>
                  <label class="text-[10px] font-bold text-slate-400 uppercase block mb-1">Description</label>
                  <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 text-sm text-slate-600 font-medium whitespace-pre-line min-h-[100px]">
                ...
                <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Created At</label>
                    <p class="text-[11px] font-bold text-slate-500">{{ viewProduct.created_at }}</p>
                  </div>
                  <div>
                    <label class="text-[10px] font-bold text-slate-400 uppercase block mb-0.5">Last Updated</label>
                    <p class="text-[11px] font-bold text-slate-500">{{ viewProduct.updated_at }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="px-8 py-6 bg-slate-50 border-t border-slate-100 flex justify-end shrink-0">
            <button @click="showViewModal=false" class="px-8 py-2.5 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 shadow-lg active:scale-95 transition-all">Close Details</button>
          </div>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="modal-fade">
      <div v-if="showModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative bg-white w-full max-w-5xl rounded-[2rem] shadow-2xl overflow-hidden animate-pop flex flex-col max-h-[90vh]">
          <div class="bg-white px-8 py-6 border-b border-slate-100 flex justify-between items-center shrink-0">
            <div class="text-left">
              <h2 class="text-xl font-black text-slate-800 tracking-tight">{{ isEdit ? 'Edit Product' : 'New Product' }}</h2>
            </div>
          ...
          <div class="p-8 space-y-4 overflow-y-auto custom-scrollbar">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1">
                ...
                <input v-model="form.name" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none" :class="{'border-rose-500': form.errors.name}"/>
                <div v-if="form.errors.name" class="text-[10px] text-rose-500 ml-1">{{ form.errors.name }}</div>
              </div>
              <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Code</label>
                <input v-model="form.product_code" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none" :class="{'border-rose-500': form.errors.product_code}"/>
                <div v-if="form.errors.product_code" class="text-[10px] text-rose-500 ml-1">{{ form.errors.product_code }}</div>
              </div>
            </div>
             ...
             <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1">
                ...
                <select v-model="form.brand_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none appearance-none" :class="{'border-rose-500': form.errors.brand_id}">
                  <option value="">Select Brand</option>
                  <option v-for="b in brands" :key="b.id" :value="b.id">{{ b.name }}</option>
                </select>
                <div v-if="form.errors.brand_id" class="text-[10px] text-rose-500 ml-1">{{ form.errors.brand_id }}</div>
              </div>
               <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Category</label>
                <select v-model="form.category_id" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none appearance-none" :class="{'border-rose-500': form.errors.category_id}">
                  <option value="">Select Category</option>
                  <option v-for="c in filteredCategories" :key="c.id" :value="c.id">{{ c.name }}</option>
                </select>
                <div v-if="form.errors.category_id" class="text-[10px] text-rose-500 ml-1">{{ form.errors.category_id }}</div>
              </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-3">
               <div class="space-y-1">
                ...
                <input v-model="form.price" type="number" step="0.01" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none" :class="{'border-rose-500': form.errors.price}"/>
                <div v-if="form.errors.price" class="text-[10px] text-rose-500 ml-1">{{ form.errors.price }}</div>
              </div>
               <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Stock</label>
                <input v-model="form.stock" type="number" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none" :class="{'border-rose-500': form.errors.stock}"/>
                <div v-if="form.errors.stock" class="text-[10px] text-rose-500 ml-1">{{ form.errors.stock }}</div>
              </div>
            </div>
             <div class="space-y-1">
              <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Description</label>
              <textarea v-model="form.description" rows="4" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none resize-none"></textarea>
            </div>
            <div class="grid grid-cols-2 gap-3">
              <div class="space-y-1">
              ...
              <div class="space-y-1">
                <label class="text-[10px] font-bold text-slate-400 uppercase ml-1">Stock Status</label>
                <select v-model="form.status_stock" class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none appearance-none">
                  <option v-for="ss in statusStocks" :key="ss.id" :value="ss.id">{{ ss.label }}</option>
                </select>
              </div>
            </div>
          </div>
